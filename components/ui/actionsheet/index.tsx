...
const actionsheetContentStyle = tva({
  base: 'bg-background web:pointer-events-auto web:select-none border-border dark:border-border/10 pb-safe max-h-[80vh] items-center rounded-t-lg border-t p-4',
});

const actionsheetItemStyle = tva({
  base: 'data-[disabled=true]:web:pointer-events-auto data-[disabled=true]:web:cursor-not-allowed data-[hover=true]:bg-accent data-[active=true]:bg-accent data-[focus=true]:bg-accent web:data-[focus-visible=true]:bg-accent w-full flex-row items-center gap-2 rounded-sm p-3 data-[disabled=true]:opacity-40',
});
...
const actionsheetDragIndicatorStyle = tva({
  base: 'bg-muted h-2 w-[100px] rounded-full',
});
...
const actionsheetBackdropStyle = tva({
  base: 'web:cursor-default web:pointer-events-auto absolute top-0 right-0 bottom-0 left-0 bg-[#000]/50',
});
...
const actionsheetSectionHeaderTextStyle = tva({
  base: 'text-muted-foreground my-0 p-3 text-xs leading-5 font-semibold uppercase',
  variants: {
    isTruncated: {
      true: '',
    },
...
const actionsheetIconStyle = tva({
  base: 'text-foreground h-4 w-4 fill-none',
});
